...
def dnatomess(dnapacket) :
    ...
    epacket = np.ones([strandsperpacket,bytesperstrand],dtype=np.uint8) # everything starts as an erasure
    for i in range(strandsperpacket) :
...
def createerrors(dnabag,srate,drate,irate) :
    ...
    newbag = np.zeros([nrows,ncols],dtype=np.uint8)
...
if __name__ == '__main__':
    ...
    badpackets = 0
    Totalbads = np.zeros(8,dtype=int)
    for ipacket in range(npackets) :
        ...
        badbytes = np.count_nonzero(messplain-messcheck)
        
        # print summary line
        Totalbads += np.array([ baddecodes,erasures,tot_detect,max_detect,tot_uncorrect,max_uncorrect,toterrcodes,badbytes])
